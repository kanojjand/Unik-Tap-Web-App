...
export default function App() {
  ...
  const handleNav = (key) => {
    setTab(key);
    setScreen({ type: key === "search" ? "list" : key === "apps" ? "applications" : "profile" });
  };
  ...
  return (
    <div style={{ maxWidth: 430, margin: "0 auto", minHeight: "100vh", background: C.bg, position: "relative" }}>
      ...
      {screen.type === "list" && <SearchScreen onSelectUni={uni => setScreen({ type: "uniDetail", uni })} />}
      ...
      <BottomNav active={tab} onNavigate={handleNav} badge={pendingCount} />
